...
export default function Showroom() {
  ...
  return (
    <section id="showroom" className="section-padding relative bg-[#FFFDF5]">
      <div className="container">
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <div className="gold-badge mb-3">
              <Car className="w-4 h-4 text-[#785E07]" />
            ...
            <p className="text-stone-600 text-sm md:text-base mt-2 max-w-xl font-semibold">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full bg-[#FEF9E7] border border-[#D4AF37]/30 rounded-xl pl-10 pr-4 py-2.5 text-sm text-stone-900 focus:outline-none focus:border-[#C5A028] transition-all font-semibold"
            />
          </div>
        </div>
            ...
            <button
              ...
              className={`px-5 py-2 rounded-xl text-xs font-extrabold tracking-wider transition-all duration-200 ${
                filterCategory === cat.id
                  ? 'bg-gradient-to-r from-[#E6C200] to-[#D4AF37] text-black shadow-lg shadow-yellow-500/25'
                  : 'bg-[#FEF9E7] text-stone-700 hover:text-stone-900 border border-[#D4AF37]/30'
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredVehicles.map((car) => (
            <div key={car.id} className="glass-card-gold overflow-hidden group hover:border-[#C5A028] transition-all duration-300 flex flex-col justify-between">
              {/* Image & Badge */}
              <div className="relative h-64 overflow-hidden bg-[#FEF9E7]">
                <img
                  src={car.image}
                  alt={car.title}
                ...
                <span className="absolute bottom-4 right-4 text-2xl font-bold font-mono text-[#FEF08A] drop-shadow-md">
                  {car.price}
                </span>
              </div>
              ...
              <div className="p-6 space-y-4">
                <h3 className="text-xl font-bold text-stone-900 font-serif group-hover:text-[#785E07] transition-colors">
                  {car.title}
                </h3>

                {/* Specs Grid */}
                <div className="grid grid-cols-2 gap-3 text-xs text-stone-800 bg-[#FEF9E7] p-3 rounded-xl border border-[#D4AF37]/30 font-bold">
                  <div className="flex items-center gap-2">
                    <Calendar className="w-3.5 h-3.5 text-[#785E07]" />
                    <span>EZ: {car.year}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Gauge className="w-3.5 h-3.5 text-[#785E07]" />
                    <span>{car.km}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Zap className="w-3.5 h-3.5 text-[#785E07]" />
                    <span>{car.power}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Fuel className="w-3.5 h-3.5 text-[#785E07]" />
                    <span>{car.fuel}</span>
                  </div>
                </div>
                ...
                <ul className="space-y-1.5 pt-1">
                  {car.features.slice(0, 3).map((feat, idx) => (
                    <li key={idx} className="text-xs text-stone-600 flex items-center gap-2 font-semibold">
      ...
      {selectedVehicle && (
        <div className="modal-overlay" onClick={() => setSelectedVehicle(null)}>
          <div 
            className="glass-card-gold max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 relative border-[#C5A028] shadow-2xl !bg-[#FFFDF5]"
          ...
          >
            ...
            {bookingSuccess ? (
              <div className="text-center py-12 space-y-4">
                <div className="w-16 h-16 bg-[#E6C200] text-black rounded-full flex items-center justify-center mx-auto animate-bounce shadow-lg">
                  <CheckCircle2 className="w-10 h-10 stroke-[3]" />
                </div>
                <h3 className="text-2xl font-bold text-stone-900 font-serif">Vielen Dank für Ihre Anfrage!</h3>
                <p className="text-stone-700 text-sm max-w-md mx-auto font-semibold">
                  Ein Berater der Schossacher Garage in Dübendorf wird sich in Kürze telefonisch unter <strong>044 820 74 74</strong> für die Bestätigung Ihrer Probefahrt des <strong className="text-[#785E07]">{selectedVehicle.title}</strong> melden.
                </p>
              </div>
            ) : (
              <div>
                <div className="relative h-64 rounded-xl overflow-hidden mb-6">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                    ...
                    <span className="text-2xl font-bold font-mono text-[#FEF08A]">{selectedVehicle.price}</span>
                  </div>
                </div>

                {/* Specs */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-[#FEF9E7] p-4 rounded-xl mb-6 text-xs text-stone-800 font-bold border border-[#D4AF37]/30">
                  <div><span className="text-stone-500 block text-[10px]">Erstzulassung</span><strong>{selectedVehicle.year}</strong></div>
                  <div><span className="text-stone-500 block text-[10px]">Kilometerstand</span><strong>{selectedVehicle.km}</strong></div>
                  <div><span className="text-stone-500 block text-[10px]">Leistung</span><strong>{selectedVehicle.power}</strong></div>
                  <div><span className="text-stone-500 block text-[10px]">Getriebe</span><strong>{selectedVehicle.transmission}</strong></div>
                </div>

                <div className="mb-6">
                  <h4 className="text-xs uppercase tracking-widest text-[#785E07] font-bold mb-3">Ausstattungshighlights</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {selectedVehicle.features.map((feat, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-xs text-stone-800 font-bold">
                        <CheckCircle2 className="w-3.5 h-3.5 text-[#785E07]" />
                        <span>{feat}</span>
                      </div>
                    ))}
                  </div>
                ...
                <form onSubmit={handleInquirySubmit} className="space-y-4 pt-4 border-t border-[#D4AF37]/20">
                  <h4 className="text-sm font-bold text-stone-900 uppercase tracking-wider">Probefahrt oder Kaufanfrage</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input type="text" required placeholder="Ihr Vor- & Nachname *" className="bg-[#FEF9E7] border border-[#D4AF37]/30 rounded-xl px-4 py-2.5 text-xs text-stone-900 focus:outline-none focus:border-[#C5A028] font-semibold" />
                    <input type="tel" required placeholder="Telefonnummer *" className="bg-[#FEF9E7] border border-[#D4AF37]/30 rounded-xl px-4 py-2.5 text-xs text-stone-900 focus:outline-none focus:border-[#C5A028] font-semibold" />
                  </div>
                  <input type="email" required placeholder="E-Mail Adresse *" className="w-full bg-[#FEF9E7] border border-[#D4AF37]/30 rounded-xl px-4 py-2.5 text-xs text-stone-900 focus:outline-none focus:border-[#C5A028] font-semibold" />

                  <div className="flex items-center justify-between pt-2">
                    <a href="tel:0448207474" className="text-xs text-[#785E07] font-bold flex items-center gap-1">
